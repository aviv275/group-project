...
class ESGAgent:
    ...
    def _analyze_text_features(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_indicators": risk_indicators if risk_indicators else ["None detected"],
            "text_risk_score": min(score, 1.0)
        }

    def _combine_results(self, text: Dict, model: Dict, rag: Dict) -> Dict[str, Any]:
        """Combines results from all analysis steps into a single risk score."""
        text_risk = text.get("text_risk_score", 0.0)
        model_risk = model.get("greenwashing_risk", 0.0)
        rag_risk = rag.get("rag_risk_score", 0.0)
        
        # Weighted average for overall risk
        overall_score = (text_risk * 0.3) + (model_risk * 0.5) + (rag_risk * 0.2)
    ...
    def _determine_fraud_alert(self, assessment: Dict) -> Dict[str, Any]:
        """Determines the final risk level and fraud alert."""
